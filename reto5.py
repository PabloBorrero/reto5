...
def leerArchivo():
    #leer el archivo .csv y lo lleva al datafile
    with open('./data.csv', 'r') as datafile: 
    # lee el archivo y lo lleva linea
        linea= datafile.readline() 
        ...
        while linea:
            ...
            # identificar posición de ciudad y departamento teniendo en cuenta que inicio en cero.
            sucursales[int(fila[5])-1]= fila[3]+ " " + fila[4]
            #
...
def main():
    ...
    for i in range(len_pacientes):
        if int(pacientes[i][i_branch])== input_central:
            entrega= False
            sis, dia = int(pacientes[i][i_ps]), int(i[pacientes][i_pd])
            if sis < 91 and dia < 63:
                entrega= True
            elif 162 <= sis < 188 and 105 <= dia < 119:
                entrega= True
    ...
    print('scheduled medicine quantity')
    print('mean {:.2f}'.format(cant_med/(hombres)))
    print(f'total:', ('paciente'))
